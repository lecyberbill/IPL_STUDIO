...
/** Run instructions per target language for the generated README. */
function runInstructionsFor(targetLang: TargetLanguage): string {
  switch (targetLang) {
    case 'python': return '```bash\npython main.py\n```';
    case 'javascript': return '```bash\nnode index.js\n```';
    case 'rust': return '```bash\ncargo run\n```';
    case 'go': return '```bash\ngo run main.go\n```';
    case 'cpp': return '```bash\ng++ -std=c++20 main.cpp -o main && ./main\n```';
    case 'html': return 'Open `index.html` in a browser (or serve it: `python -m http.server`).';
    default: return 'See the generated entry file for how to run this project.';
  }
}

/** Builds a tracking README for the generated project (never the source of truth — that is `source/main.ipl`). */
function buildProjectReadme(projectName: string, targetLang: TargetLanguage): string {
  return [
    `# ${projectName}`,
    '',
    'Generated by **IPL Studio** — intent-to-application pipeline.',
    '',
    `- **Target**: \`${targetLang}\``,
    `- **Generated**: ${new Date().toISOString()}`,
    '- **Spec**: `source/main.ipl` (the IPL source of truth — regenerate from it to reproduce this project)',
    '',
    '## Run',
    '',
    runInstructionsFor(targetLang)
  ].join('\n');
}
...
export function buildProjectArtifact(
  ...
  if (files.length === 0) {
    if (targetLang === 'rust') {
      files.push({
        ...
        content: `[package]\nname = "${safeName}"\nversion = "0.1.0"\nedition = "2021"\nauthors = ["IPL Studio v1.4.0 <atelier@ipl.io>"]\n\n[dependencies]\ntokio = { version = "1.0", features = ["full"] }\nserde = { version = "1.0", features = ["derive"] }\nserde_json = "1.0"\n`
      });
      files.push({
      ...
      files.push({
        ...
        content: `# Rust Project - ${projectName}\n\nArtifact generated by **IPL Studio v1.4.0**.\n\n## Run\n\`\`\`bash\ncargo run\n\`\`\`\n`
      });
      files.push({
  ...
  // Tracking README always included (unless the model already emitted one) —
  // points to source/main.ipl so the project stays reproducible.
  if (!files.some(f => /readme\.md$/i.test(f.relativePath))) {
    files.push({
      relativePath: 'README.md',
      content: buildProjectReadme(projectName, targetLang)
    });
  }
